...
berries = [int(i) for i in input(
    "Введите количество ягод на кустах: ").split()[:n + 1]]
if (len(berries) < 4):
    sum = 0
    for i in berries:
        sum += i
    print(f"Максимальное количество ягод = {sum}")
else:
    max_berries = berries[-2] + berries[-1] + berries[0]
    print(max_berries)
